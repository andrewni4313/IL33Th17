...
# Load Data
sds <- slingshot(Embeddings(tc.cd4, "umap"), clusterLabels = tc.cd4$seurat_clusters, 
...
cell_colors_clust <- cell_pal(tc.cd4$cd4type, hue_pal())
cell_colors_tumor <- cell_pal(tc.cd4$tumor, hue_pal())
...
gene_annotation <- as.data.frame(rownames(tc.cd4@reductions[["pca"]]@feature.loadings), row.names = rownames(tc.cd4@reductions[["pca"]]@feature.loadings))
colnames(gene_annotation) <- "gene_short_name"

# part two, cell information
cell_metadata <- as.data.frame(tc.cd4@assays[["RNA"]]@counts@Dimnames[[2]], row.names = tc.cd4@assays[["RNA"]]@counts@Dimnames[[2]])
colnames(cell_metadata) <- "barcode"

# part three, counts sparse matrix
New_matrix <- tc.cd4@assays[["RNA"]]@counts
New_matrix <- New_matrix[rownames(tc.cd4@reductions[["pca"]]@feature.loadings), ]
...
list_cluster <- tc.cd4@meta.data[[sprintf("cd4type")]]
names(list_cluster) <- tc.cd4@assays[["RNA"]]@data@Dimnames[[2]]
cds@clusters@listData[["UMAP"]][["clusters"]] <- list_cluster

rm(list_cluster)  # Cleanup

pData(cds)$cellType <- tc.cd4@meta.data$cellType
...
cds@reduce_dim_aux@listData[["UMAP"]] <- tc.cd4@reductions[["umap"]]@cell.embeddings
cds@int_colData@listData[["reducedDims"]]@listData[["UMAP"]] <- tc.cd4@reductions[["umap"]]@cell.embeddings
...
cds@preprocess_aux$gene_loadings <- tc.cd4@reductions[["pca"]]@feature.loadings
...
ordering_genes <- FindAllMarkers(tc.cd4, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.5)
...
pca_time <- data.frame(x = tc.cd4@reductions[["pca"]]@cell.embeddings[,1], y = tc.cd4$cd4type, colour = tc.cd4$cd4type)
...
```{r}
save(cd,file="Data/Cells/CD4/cd.Robj")
save(BEAM_res,file="Data/Cells/CD4/BEAM_res.Robj")
save(pseudotime_de,file="Data/Cells/CD4/pseudotime_de.Robj")
